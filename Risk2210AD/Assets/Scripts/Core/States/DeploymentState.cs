    ...
    public sealed class DeploymentState : PhaseState
    {
        ...
        public override CommandResult Handle(GameCommand cmd)
        {
            ...
            switch (cmd)
            {
                ...
                case PlayCard pc:
                    if (ps.Pool > 0) return Fail("deploy all your MODs first");
                    return PlayBeforeInvasionCard(D, p, pc.HandIndex);
        ...
        /// <summary>Shared by Deployment and Invasion (cards may be played until the first invasion is declared).</summary>
        public static CommandResult PlayBeforeInvasionCard(GameDirector D, int p, int handIndex)
        {
            var S = D.State;
            var ps = S.Players[p];
            ...
            if (card.Timing != CardTiming.BeforeFirstInvasion) return CommandResult.Fail("that card cannot be played now");
            if (ps.InvasionDeclaredThisTurn) return CommandResult.Fail("you have already declared an invasion this turn");
            if (!S.CommanderInPlay(p, card.Deck)) return CommandResult.Fail(card.Deck + " Commander is not in play");
            if (ps.Energy < card.Cost) return CommandResult.Fail("not enough energy");
            if (card.Cost > 0) D.ChangeEnergy(p, -card.Cost);
